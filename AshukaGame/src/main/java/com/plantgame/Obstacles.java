...
public class Obstacles {
	...
	private final Point[] obs_std = new Point[PGConstants.OBS_NUM];
	private final double[] obs_dgr = new double[PGConstants.OBS_NUM];
	private final int OBS_WIDTH;
	private final int OBS_HEIGHT;
	
	public Obstacles(String imgPath) throws IOException {
		...
		for(int i = 0; i < PGConstants.OBS_NUM; ++i) {
			...
			while(true) {
				if(	obs_std[i].getX() > (double) PGConstants.WIDTH_MID / 2 &&
						obs_std[i].getX() < (double) (PGConstants.WIDTH_MID * 3) / 2 &&
						obs_std[i].getY() > (double) (PGConstants.HEIGHT * 2) / 3
					) {
					obs_std[i] = new Point(
							(int)((PGConstants.X_RANGE - OBS_WIDTH) * Math.random() + PGConstants.LEFT_RIGHT_BOTTOM),
							(int)((PGConstants.Y_RANGE - OBS_HEIGHT) * Math.random() + PGConstants.TOP)
